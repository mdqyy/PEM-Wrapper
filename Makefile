...
cmake: 
	if [ -d $(BIN_DIR) ]; then cd $(BIN_DIR) && cmake -DCMAKE_CXX_COMPILER=$(CXX) -DCMAKE_C_COMPILER=$(CC) -DCMAKE_INSTALL_PREFIX=$(INSTALL_PREFIX) ../../; fi
debug: 
...
clean:
	if [ ! -d $(BIN_DIR) ]; then mkdir -pv $(BIN_DIR); fi && if [ ! -f $(BIN_DIR)/Makefile ]; then cd $(BIN_DIR) && cmake -DCMAKE_CXX_COMPILER=$(CXX) -DCMAKE_C_COMPILER=$(CC) -DCMAKE_INSTALL_PREFIX=$(INSTALL_PREFIX) ../../ && cd $(PWD); fi && cd $(BIN_DIR) && make clean
